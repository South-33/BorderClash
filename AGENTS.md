...
- Dashboard data path lives in `convex/api.ts` + `convex/research.ts` + `src/lib/convex-server.ts` + `src/app/DashboardClient.tsx` -> publish the singleton `dashboardSnapshots` doc after synthesis and keep `getDashboardSnapshot` as the canonical ISR/live read, or server/client drift will reintroduce stale partial renders.
